...
const navItems = [{"id":"1","label":"Home","url":"/","icon":"home"},{"id":"1769104834626","label":"Shop","url":"/categories/lcdtouch-folder-combo/products","icon":"cart"},{"id":"1769104835750","label":"Account","url":"/account","icon":"user"}];
const BASE_DOMAIN = 'favstore24.com';
      ...
      <View style={styles.browserContainer}>
        ...
        {browserLoading && (
          <View style={styles.browserLoading}>
            <ActivityIndicator size="small" color="#f7f7f8" />
          </View>
        )}
        <WebView
          ref={browserRef}
    ...
    >
      {navItems.map((item, index) => (
        <Tab.Screen 
          key={index}
          name={item.label} 
          children={() => <WebViewScreen url={"https://favstore24.com/en" + item.url} />}
        />
      ))}
    </Tab.Navigator>
  );
